...
#            kernel = tf.get_variable("W", trainable=False, initializer=W)
#            biases = tf.get_variable("b", trainable=False, initializer=b)
            conv = tf.nn.conv2d(prev_layer, tf.constant(W), strides=(1, 1, 1, 1), padding='SAME')
        out = tf.nn.relu(conv + tf.constant(b), name=scope.name)
        ###############################
