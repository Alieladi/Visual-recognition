...
Modified by Ali El Adi (alieladi@gmail.com)
...
class StyleTransfer(object):
    ...
    def _content_loss(self, P, F):
        ...
        with tf.name_scope('content_loss'):
            s = tf.cast(tf.reduce_prod(P.shape), tf.float32) # = P.size
            loss = tf.square(P - F)
            self.content_loss = tf.reduce_sum(loss) / (4 * s)
        ###############################
    ...
    def _single_style_loss(self, a, g):
        ...
        """
        ###############################
        M = g.shape[1]*g.shape[2] # height*width
        N = g.shape[3] # filters
        G = self._gram_matrix(g, N, M)
        A = self._gram_matrix(a, N, M)
        loss = tf.square(G - A)
        return tf.reduce_sum(loss) / (4*(N.value**2)*(M.value**2)) # value or g.get_shape().as_list()
        ###############################
    ...
    def train(self, n_iters):
        ...
        with tf.Session() as sess:
            ...
            writer = tf.summary.FileWriter('graphs/style_transfer_cor', sess.graph)
            ###############################
            ...
            for index in range(initial_step, n_iters):
                ...
                sess.run(self.opt) # optimization run
                if (index + 1) % skip_step == 0:
                    ...
                    if (index + 1) % 20 == 0:
                        ...
                        saver.save(sess, 'checkpoints/style_transfer_cor/style_transfer', index)
                        ###############################
                        pass

if __name__ == '__main__':
    setup()
    machine = StyleTransfer('content/morocco-tour.png', 'styles/harlequin.jpg', 333, 250)
    machine.build()
    machine.train(300)
